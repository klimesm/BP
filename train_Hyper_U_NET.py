...
def unet1(input_size):
    ...
    model.compile(optimizer = Adam(learning_rate = 1e-4), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    return model
...
while((i<epochs_max) and (stop==0)):
    ...
    if(iter1==1):
        model1.compile(optimizer = Adam(learning_rate = 5e-5), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    if(iter1==2):
        model1.compile(optimizer = Adam(learning_rate = 2e-5), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    if(iter1==4):
        model1.compile(optimizer = Adam(learning_rate = 1e-5), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    if(iter1==8):
        model1.compile(optimizer = Adam(learning_rate = 5e-6), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    if(iter1==16):
        model1.compile(optimizer = Adam(learning_rate = 2e-6), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    if(iter1==32):
        model1.compile(optimizer = Adam(learning_rate = 1e-6), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    if(iter1==64):
        model1.compile(optimizer = Adam(learning_rate = 5e-7), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    if(iter1==128):
        model1.compile(optimizer = Adam(learning_rate = 2e-7), loss = 'mean_absolute_error', metrics = ['RootMeanSquaredError'])
    if(iter1==256):
